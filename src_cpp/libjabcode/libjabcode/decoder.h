...
namespace jabcode {
	...
	*/
	...
	__declspec(dllexport) jab_int32 __stdcall decodeMaster(jab_bitmap* matrix, jab_decoded_symbol* symbol);
	__declspec(dllexport) jab_int32 __stdcall decodeSlave(jab_bitmap* matrix, jab_decoded_symbol* symbol);
	__declspec(dllexport) jab_data* __stdcall decodeData(jab_data* bits);
	__declspec(dllexport) void      __stdcall deinterleaveData(jab_data* data);
	__declspec(dllexport) void      __stdcall getNextMetadataModuleInMaster(jab_int32 matrix_height, jab_int32 matrix_width, jab_int32 next_module_count, jab_int32* x, jab_int32* y);
	__declspec(dllexport) void      __stdcall demaskSymbol(jab_data* data, jab_byte* data_map, jab_vector2d symbol_size, jab_int32 mask_type, jab_int32 color_number);
	__declspec(dllexport) jab_int32 __stdcall readColorPaletteInMaster(jab_bitmap* matrix, jab_decoded_symbol* symbol, jab_byte* data_map, jab_int32* module_count, jab_int32* x, jab_int32* y);
	__declspec(dllexport) jab_int32 __stdcall readColorPaletteInSlave(jab_bitmap* matrix, jab_decoded_symbol* symbol, jab_byte* data_map);
}
#endif
