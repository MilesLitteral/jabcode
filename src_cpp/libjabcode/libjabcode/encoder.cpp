...
namespace jabcode {
	...
	*/
	void getColorPaletteIndex(jab_byte* index, jab_int32 index_size, jab_int32 color_number)
	{
		...
		jab_byte** tmp = new jab_byte* [color_number];
		for(jab_int32 i=0; i<color_number; i++)
		{
			tmp[i] = new jab_byte[color_number];
		}
	...
	*/
	jab_boolean createMatrix(jab_encode* enc, jab_int32 index, jab_data* ecc_encoded_data)
	{
		...
		jab_byte** palette_index = new jab_byte* [palette_index_size];
		for (int i = 0; i < palette_index_size; i++) {
			palette_index[i] = new jab_byte[palette_index_size];
		}
	...
	*/
	jab_boolean fitDataIntoSymbols(jab_encode* enc, jab_data* encoded_data)
	{
		...
		jab_int32* capacity     = new jab_int32 [enc->symbol_number]; 		//This is causing a SegFault
		jab_int32* net_capacity = new jab_int32 [enc->symbol_number];
		jab_int32   total_net_capacity = 0;
		for(jab_int32 i=0; i<enc->symbol_number; i++)
		{
			capacity[i] = getSymbolCapacity(enc, i); //This is causing a SegFault
		...
		for(jab_int32 i=0; i<enc->symbol_number; i++)
		{
			...
			{
				jab_float prop = (jab_float)(net_capacity[i] / total_net_capacity);
...
*/
	...
	*/
	jab_int32 generateJABCode(jab_encode* enc, jab_data* data)
	{
		...
		{
			printf("ERROR: Auto Assigning Fit!\n");
			free(encoded_data);
			return 4;
		}
		free(encoded_data);
