...
jab_boolean binarizerRGB(jab_bitmap* bitmap, jab_bitmap* rgb[3], jab_float* blk_ths)
{
    ...
    jab_float** pixel_ave = new jab_float * [block_num_x * block_num_y];
...
jab_int32 selectBestPatterns(jab_finder_pattern* fps, jab_int32 fp_count, jab_int32* fp_type_count)
{
    ...
    jab_finder_pattern** fps0 = new jab_finder_pattern*[fp_type_count[FP0]];
    jab_finder_pattern** fps1 = new jab_finder_pattern*[fp_type_count[FP1]];
    jab_finder_pattern** fps2 = new jab_finder_pattern*[fp_type_count[FP2]];
    jab_finder_pattern** fps3 = new jab_finder_pattern*[fp_type_count[FP3]];
    ...
    for(jab_int32 j=0; j<ch[0]->width && done == 0; j+=min_module_size)
    {
        ...
        do
        {
            ...
            if(seekPattern(ch[1], -1, j, &starty, &endy, &centery_g, &module_size_g, &skip))
            {
                ...
                if(crossCheckPatternVertical(ch[2], (jab_int32)module_size_g*2, (jab_float)j, &centery_b, &module_size_b))
                {
                    type_b = ch[2]->pixel[(jab_int32)(centery_b)*ch[2]->width + j] > 0 ? 255 : 0;
                    //check red channel
                    module_size_r = module_size_g;
                    jab_int32 core_color_in_red_channel = jab_default_palette[FP3_CORE_COLOR * 3 + 0];
                    if(crossCheckColor(ch[0], core_color_in_red_channel, (jab_int32)module_size_r, 5, j, (jab_int32)centery_r, 1))
                    {
                    	type_r = 0;
                ...
                else if(crossCheckPatternVertical(ch[0], (jab_int32)module_size_g*2, (jab_float)j, &centery_r, &module_size_r))
				{
					type_r = ch[0]->pixel[(jab_int32)(centery_r)*ch[0]->width + j] > 0 ? 255 : 0;
					//check blue channel
					module_size_b = module_size_g;
					jab_int32 core_color_in_blue_channel = jab_default_palette[FP2_CORE_COLOR * 3 + 2];
                    if(crossCheckColor(ch[2], core_color_in_blue_channel, (jab_int32)module_size_b, 5, j, (jab_int32)centery_b, 1))
                    {
                    	type_b = 0;
...
jab_bitmap* sampleSymbolByAlignmentPattern(jab_bitmap* bitmap, jab_bitmap* ch[], jab_decoded_symbol* symbol, jab_finder_pattern* fps)
{
    ...
    jab_vector2d** rect = new jab_vector2d * [(block_number * 2)];
